 ...
 *
 * The default formatter is pinned to en-US, so every expectation below is also a check
 * that the host machine's locale does not leak into CI output.
 *
...
describe('the Hermes ICU fallback (§6)', () => {
  it('still groups when Intl is missing entirely', () => {
    const globals = globalThis as { Intl?: typeof Intl };
    const original = globals.Intl;
    try {
      globals.Intl = undefined;
      const withoutIntl = createStatFormatter('en-US');
