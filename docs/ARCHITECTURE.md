...
| Backup           | **`expo-sharing`** + `expo-file-system`                                              | The roster's only copy leaves through the system share sheet and comes back through `File.pickFileAsync` (ADR-0033). No `expo-document-picker`: the picker is already in `expo-file-system`. `expo-sharing`'s config plugin is **not** declared — it is for receiving shares, which this app does not do |
...
src/
  app/                      Expo Router routes — thin. Screens live in features/
    ...
    me.tsx                  -> features/playerForm + features/rosterBackup
  ...
  core/data/                Repositories, sync orchestration. The only place that knows
                            both the database and the network exist. Also the backup
                            document and its reader (ADR-0033), behind a BackupFile port
                            whose device adapter is the share sheet and the file picker.
  ...
  features/rosterBackup/    RosterBackupControls, useRosterBackup — the roster off this
                            device and back (ADR-0033). Rendered by /me, which composes
                            it with playerForm because a feature may not import another.
...
export interface RosterRepository {
  ...
  // ADR-0033. The roster's only copy can leave the device, and come back.
  exportSnapshot(): RosterBackup;
  importSnapshot(text: string): Result<RosterBackupSummary>;
}
```
...
**The roster can leave the device (ADR-0033).** Everything above writes to one app-private SQLite
file on one phone, so an uninstall or a "clear data" tap is total loss. `exportSnapshot` reads the
whole ladder into a schema-versioned JSON document and the share sheet takes it; `importSnapshot`
reads one back, and it **replaces** rather than merging — the counterpart to `replaceRoster`, and
deliberately not the same function, because a sync is a partial view and a restore is not. The
document carries each row's `origin`, which is the one thing `RosterSnapshot` cannot say: a snapshot
comes from a server that has no opinion about which rows this device typed in, so restoring through
one would hand back a roster nobody may edit. Every refusal happens in `parseRosterBackup`, before
the writer opens its transaction, because that transaction's first statement deletes the ladder.
